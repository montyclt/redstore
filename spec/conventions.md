...
├── datagen/                           # client source set: recipes, loot, tags, block states
│   ├── RedstoreDataGenerator.java
│   ├── RedstoreRecipes.java
│   ├── RedstoreLootTables.java
│   ├── RedstoreBlockTags.java
│   └── RedstoreModels.java
├── registry/
...
Two language files are maintained from the start, **by hand** — see section 11.1 for why they are
the one thing datagen does not touch:
...
The mod writes its own data files, with `./gradlew runDatagen`, into **`src/main/generated`**,
which is committed and is a resource directory of the main source set. The entry point is
`net.montyclt.redstore.datagen.RedstoreDataGenerator`.

| Generated | Not generated |
| --- | --- |
| Recipes, and the unlock advancement of each | The two language files |
| Loot tables | The five item sprite models (`models/item/`) |
| Tags — the two families and `mineable/pickaxe` | Everything in section 10: textures and block models |
| Block state files | |
| Item definitions (`items/`) | |

**The advancements are why this exists.** A recipe without its unlock advancement is craftable and
never appears in the crafting table's book; that was shipped broken once, found in game, and fixed
by hand. Written by hand it is a file you can forget. Generated it is not a file at all: the recipe
builder emits it.

**The block state files are the second reason.** A clock has 128 states and a gate 32. Those were
written by throwaway scripts that lived on nobody's machine but the author's, which is the worst of
both worlds — generated, and not reproducible.

### 11.1 What is deliberately left out

* **The language files.** A translator edits JSON; putting a generator between them and the text
  buys nothing and costs everyone who is not a Java programmer.
* **The item sprite models.** Three lines each of `item/generated`, and they never change.
* **Everything in section 10.** Those are derived from *vanilla's own files*, not from our
  registry, and they are art rather than data. Two generators, two jobs: `generateAssets` builds
  what the blocks look like, datagen writes what the game must know about them. The block state
  files are the seam — they name models that `generateAssets` produces, which is why every model
  in that provider is a string.

### 11.2 Two things the generator has to be told

* **Item definitions point at the sprite models, not at the block models.** Left alone the
  generator points each item at its block, and a two-pixel plate rendered in three dimensions is a
  sliver nobody can read in a hotbar — which is why vanilla draws repeaters and comparators as
  sprites. The provider says so explicitly, for every item.
* **`inverted` is not dispatched on.** It changes nothing to look at, so the gates' block state
  files do not mention it, which is both the truth and half the file.
